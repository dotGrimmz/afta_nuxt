...
const computeDpr = () => {
  ...
  const clamped = window.innerWidth < 768 ? Math.min(base, 1.2) : base;
  return clamped * props.resolutionScale;
};
...
const resize = () => {
  ...
  if (!height || height === 0) {
    ...
    height = rect.height || Math.max(width * 0.6, 1);
  }

  renderer.dpr = computeDpr();
  renderer.setSize(width, height);

  const gl = renderer.gl;
  const actualWidth = gl.drawingBufferWidth;
  const actualHeight = gl.drawingBufferHeight;
  program.uniforms.uResolution.value.set(actualWidth, actualHeight);
};
...
onMounted(() => {
  ...
  orientationHandler = () => requestAnimationFrame(resize);
